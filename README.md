...
| 通道 | 接口 | 用途 |
| --- | --- | --- |
| `PXI2Slot3/ao1` | 功放 `Start` | 与 TEM 完全一致：`1 ms` 的 `0 V` 后输出 `9 ms` 的 `4 V` |
...
时域电流图和幅相窗口均使用换算后的安培值，`Rx/Current` 的幅值单位为 `V/A`。每次测量会在 `*_info.json` 中保存当时使用的衰减倍数。
...
- `*_t.npy`：时间轴，单位秒；
- `*_rx.npy`：`ai31` 原始接收电压；
- `*_current.npy`：`ai29` 原始电流监测电压，单位 `V`；显示和分析时再乘以记录的衰减倍数换算为 `A`；
- `*_info.json`：发射、采样和波形参数。
