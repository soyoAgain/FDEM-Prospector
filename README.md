...
| `PXI2Slot2/ai29` | 电流监测 | 通过衰减探头测量与发射线圈串联的 `0.47 Ohm` 采样电阻两端电压 |
...
`ai29` 通过衰减探头测量与发射线圈串联的 `0.47 Ohm`（电阻标记 `R47`）采样电阻两端电压。界面中的采样电阻和探头衰减倍数用于换算：

```text
发射电流 [A] = ai29 原始采集电压 [V] x 探头衰减倍数 / 采样电阻 [Ohm]
```

默认采样电阻为 `0.47 Ohm`，默认探头衰减倍数为 `500`。时域电流图和幅相窗口均使用换算后的安培值，`Rx/Current` 的幅值单位为 `V/A`。每次测量会在 `*_info.json` 中保存采样电阻和探头衰减倍数；历史记录缺少采样电阻字段时按 `0.47 Ohm` 解释。
...
- `<测点>_<编号>_<频率>Hz_current.npy`：`ai29` 经衰减探头采集的电压，单位 `V`；显示和分析时先乘探头衰减倍数，再除以采样电阻值换算为 `A`；
